...
export class UserService {
    async userPurchases(userId: string) {
        ...
        const allPurchases = await db.select().from(payments).where(eq(payments.user_id, UserObj.id));
    ...
    async userSignup(data: any) {
        ...
        const [isUserExist] = await db.select().from(users).where(eq(users.email, email)).limit(1);
    ...
    async generateEmailValidationToken(email: string) {
        const [UserObj] = await db.select().from(users).where(eq(users.email, email)).limit(1);
        ...
        }).where(eq(users.id, UserObj.id)).returning();

        if (!update) {
            throw new CustomError("token not set");
        }

        const message = buildEmailNotification('token', 'success', `Your validation token is ${token}`, "User email validation")
    ...
    async verifyUserEmailValidationToken(token: string, email: string, userId?: string) {
        return await db.transaction(async (tx) => {
            ...
            if (userId) {
                [UserObj] = await tx.select().from(users).where(and(
                    eq(users.id, userId),
            ...
            const [existingSocial] = await tx.select().from(socials).where(and(
                eq(socials.user_id, UserObj.id),
                eq(socials.platform, "email")
            )).limit(1);

            if (existingSocial) {
                await tx.update(socials).set({
                    is_verified: true,
                    updated_at: new Date()
    ...
    async generateUserTelegramIdValidationToken(userId: string, telegramid: string) {
        const [telegram] = await db.select().from(socials).where(and(
            eq(socials.user_id, userId),
            eq(socials.platform, "telegram")
        )).limit(1);
...
`;
        ...
        const message = buildTelegramNotification('token', 'success', MESSAGE, "Telegram Token")
    ...
    async verifyUserTelegramIdValidationToken(userId: string, token: string) {
        return await db.transaction(async (tx) => {
            ...
            const [UserObj] = await tx.select().from(users).where(and(
                eq(users.id, userId),
                eq(users.forgot_password_token, token_hash)
            )).limit(1);
    ...
    async generateUserForgotPasswordToken(email: string) {
        ...
        const message = buildEmailNotification('token', 'success', `Your reset password token is ${token}`, "Reset Password")
        MessageDispatcher.dispatch(message, email)
    ...
    async verifyUserForgotPasswordToken(data: any) {
        ...
        const [UserObj] = await db.select().from(users).where(and(
            eq(users.email, email),
            eq(users.forgot_password_token, token_hash)
        )).limit(1);
    ...
    async userSignin(data: any) {
        ...
        const veryfypassword = await veryfyhashPasswordFn(password, UserObj.password);
    ...
    async updateUser(userId: string, data: any) {
        ...
        if (data.academicProfile) updateData.academic_profile = data.academicProfile;
    ...
    async getUserTimeline(userId: string) {
        const [user] = await db.select({ exam_year_id: users.exam_year_id }).from(users).where(eq(users.id, userId)).limit(1);
        ...
        const timelineEvents = await db.select().from(exam_timelines).where(eq(exam_timelines.exam_year_id, user.exam_year_id)).orderBy(exam_timelines.date);
    ...
    async getSubscriptionTiers() {
        ...
        const allBenefits = tierIds.length > 0 ? await db.select().from(tier_benefits).where(inArray(tier_benefits.tier_id, tierIds)) : [];
    ...
    async getUserSubscriptionDetails(userId: string) {
        ...
        const [tierData] = await db.select().from(tiers).where(eq(tiers.name, currentStatus)).limit(1);
        let tier = null;
        if (tierData) {
            const benefits = await db.select().from(tier_benefits).where(eq(tier_benefits.tier_id, tierData.id));
