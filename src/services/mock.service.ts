...
export class MockService {
    ...
    async deleteQuestionsFromMock(mockid: string) {
        const [status] = await db.delete(question_maps).where(eq(question_maps.exam_id, mockid)).returning();
    ...
    async getMockById(id: string) {
        ...
        const [{ value: qCount }] = await db.select({ value: count() }).from(question_maps).where(eq(question_maps.exam_id, id));
    ...
    async addQuestionToMock(mockId: string, questionId: string, part: string = "part1", number: number) {
        const [response] = await db.insert(question_maps).values({
            exam_id: mockId,
    ...
    async removeQuestionFromMock(mockId: string, questionId: string) {
        const response = await db.delete(question_maps).where(
            and(
    ...
    async getExamPatterForMock(mockId: string) {
        ...
        if (!response || !response.exam_pattern) throw new Error("Mock Set not found");
    ...
    async getMockQuestions(mockId: string, info: "full" | "Onlyid") {
        const [isMock] = await db.select({ id: exams.id }).from(exams).where(
            and(
                eq(exams.exam_type, "Mock"),
                eq(exams.id, mockId)
            )
        ...
        if (info === "Onlyid") {
            questionList = await db.select().from(question_maps).where(eq(question_maps.exam_id, mockId));
        } else {
            questionList = await db.select({
                id: question_maps.id,
                number: question_maps.number,
                question_id: question_maps.question_id,
                part: question_maps.part,
