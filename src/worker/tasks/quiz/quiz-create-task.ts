...
export class QuizCreateTask extends BaseWorkerTask {
  ...
  async execute(): Promise<void> {
    ...
    let quizData: QuizMetaData | null = await this.getQuizMetaData(
      this.task.payload.quizId,
    );
  ...
  async changeQuizDataStatus(
    quizData: QuizMetaData,
    status: CreationTypes = "Processing",
  ) {
    quizData.status = status;
    await this.redisManager
      .getclient()
      .set(`quiz:data:${quizData.id}`, JSON.stringify(quizData), "KEEPTTL");
  }
  ...
  async addQuestionsToRedis(examId: string, questions: quiz_question_type[]) {
    const formattedQuestions: exam_question_format_type[] = questions.map(
      (question, number) => {
