...
exports.bulk_mock_questions_set_Create = exports.sendBulkMockAndPyqData = exports.changeUserid = void 0;
const index_1 = __importDefault(require("@repo/db/index"));
const fs_1 = __importDefault(require("fs"));
const changeUserid = (userid, data) => {
    let count = 0;
...
const sendBulkMockAndPyqData = async (inputepath, userid) => {
    ...
    if (data.mock_set) {
        ...
        await run(data.mock_set, exports.bulk_mock_questions_set_Create);
    }
    console.log("mock_question_set added....");
    await new Promise((resolve) => setTimeout(resolve, 4000));
    if (data.test) {
        let processedData = (0, exports.changeUserid)(userid, data.test);
        // await run(processedData, bulkMockAndPyqCreate);
    }
};
exports.sendBulkMockAndPyqData = sendBulkMockAndPyqData;
const run = async (data, fn) => {
...
//   return results;
// };
const bulk_mock_questions_set_Create = async (bulkData) => {
    const result = await index_1.default.question_map.createMany({
        data: bulkData,
...
exports.bulk_mock_questions_set_Create = bulk_mock_questions_set_Create;
