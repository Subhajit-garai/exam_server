...
export class QuestionService {
  async updateQuestion(userId: string, data: any) {
    ...
    const updatePayload: any = {
      ...
      ...(links && { links: links }),
      ...(is_multiple_ans !== undefined && {
        is_multiple_answers: is_multiple_ans,
      }),
  ...
  async deleteQuestion(userId: string, questionId: string) {
    ...
    const [{ value: usageInExams }] = await db
      .select({ value: count() })
      .from(question_maps)
      .where(eq(question_maps.question_id, questionId));
    if (usageInExams > 0) {
      throw new Error(
        `Cannot delete: Question is used in ${usageInExams} exam(s).`,
      );
    }

    const [{ value: usageInQuizzes }] = await db
      .select({ value: count() })
      .from(quiz_question_maps)
      .where(eq(quiz_question_maps.question_id, questionId));
    if (usageInQuizzes > 0) {
      throw new Error(
        `Cannot delete: Question is used in ${usageInQuizzes} quiz(zes).`,
      );
    }

    const [{ value: userAnswersCount }] = await db
      .select({ value: count() })
      .from(user_answers)
      .where(eq(user_answers.question_id, questionId));
    if (userAnswersCount > 0) {
      throw new Error(
        `Cannot delete: Question has ${userAnswersCount} user answers recorded.`,
      );
    }
  ...
  async checkQuestion(title: string) {
    const response = await db
      .select()
      .from(questions)
      .where(ilike(questions.title, `%${title}%`));
    return response;
  }
  ...
  async getAllQuestions(filters: any, page: number = 1) {
    ...
    if (id) {
      ...
      if (title && title.trim())
        conditions.push(ilike(questions.title, `%${title.trim()}%`));
      if (ismultipleans !== undefined)
        conditions.push(eq(questions.is_multiple_answers, ismultipleans));
  ...
  async getSubjectCounts(category?: string) {
    ...
    const conditions = [];
    if (categoryIdFilter)
      conditions.push(eq(questions.category_id, categoryIdFilter));
...
export class QuestionProcessingService {
  ...
  async getProcessedQuestions(filters: any, page: number = 1) {
    ...
    const conditions = [];
    if (status)
      conditions.push(eq(question_processing.processing_status, status as any));
    if (topic_id) conditions.push(eq(question_processing.topic_id, topic_id));
    if (subject_id)
      conditions.push(eq(question_processing.subject_id, subject_id));
  ...
  async reviewQuestion(
    ...
    const [processedQuestion] = await db
      .select()
      .from(question_processing)
  ...
  async deleteProcessedQuestion(adminId: string, questionProcessingId: string) {
    ...
    if (!admin || admin.role !== "Admin") {
      throw new Error(
        "Unauthorized: Only admins can delete processed questions",
      );
    }

    await db
      .delete(question_processing)
      .where(eq(question_processing.id, questionProcessingId));

    return { success: true };
  }

  async updateProcessedQuestion(
    adminId: string,
    questionProcessingId: string,
    data: any,
  ) {
    ...
    if (!admin || admin.role !== "Admin") {
      throw new Error(
        "Unauthorized: Only admins can update processed questions",
      );
    }
