...
-------------------------------------*/
import { eventRuns, eventType, ExamType, primeStatus, purchaseType, UserRole, Visibility, } from "./generated/prisma/enums";
import fs from "fs";
...
export const getDiscountPercent = (markedPrice, price) => {
    return ((markedPrice - price) / markedPrice) * 100;
};
export async function createOrUpdateTier(tierName, benefits) {
    ...
    if (!existing) {
        ...
        const updates = await Promise.all(benefits.map((b) => prisma.tierBenefit.upsert({
            where: {
                tierId_feature: {
                    tierId: existing.id,
                    feature: b.feature,
...
export const sendBulkQuestionData = async (inputepath, userid, subjectid, topicid) => {
    ...
    if (length > chunkSize) {
        ...
        for (let index = 0; index < length; index += chunkSize) {
            ...
            console.log(`Inserting chunk ${index / chunkSize + 1} — records ${index}..${index + chunk.length - 1}`);
...
export const changeUserid = (userid, data, subjectid, topicid) => {
    let count = 0;
...
export const bulkQuestionCreate = async (bulkData) => {
...
const addevent = async (botid, exam_pattern_id) => {
    // Create a set of daily events for the bot
    let Create_exam_payload = {
        starttime: ["8:00 am"],
        count: "1",
        title: "autoincrement",
        examname: "JECA",
...
async function main() {
    ...
    await sendBulkQuestionData(path.resolve("seeds", "data", "question.json"), user1.id, unknownSubject.id, unknownTopic.id);
}
/* ----------------------------------
