...
export class PaymentService {
    ...
    private async isPaymentProcessed(paymentId: string) {
        const [pay] = await db.select({ id: payments.id })
            .from(payments)
    ...
    async checkoutSubcription(userId: string, plan: primeStatus, amount: string, type: string, couponCode?: string) {
        ...
        const [isPlanExist] = await db.select().from(subscription_offers).where(
            and(
        ...
        const [tierInfo] = await db.select().from(tiers).where(eq(tiers.name, plan));
        ...
        return await db.transaction(async (tx) => {
            ...
            if (isAuthentic) {
                ...
                switch (type as purchaseType) {
                    case "SUBSCRIPTION":
                        {
                            const amt = typeof amount == "string" ? parseInt(amount) : (amount as number);

                            const [userOrder] = await tx.update(orders)
                            ...
                            await ProvideSubcriptionTouser(userid, subscription as primeStatus, orderDetails);
                        }
                        break;
                    case "TOKEN":
                        {
    ...
    async getSubcriptionAndOfferFormated() {
        const rows = await db.select({
            ...
            .leftJoin(target_exams, eq(subscription_offers.target_exam_id, target_exams.id));
