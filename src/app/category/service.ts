...
export class CategoryService {
  async createCategory(data: any) {
    ...
    const [existing] = await db
      .select()
      .from(categories)
      .where(or(eq(categories.name, data.name), eq(categories.slug, slug)))
      .limit(1);
  ...
  async getCategoryById(id: string) {
    ...
    const examPatterns = await db
      .select()
      .from(exam_patterns)
      .where(eq(exam_patterns.category_id, id));
    const targetExams = await db
      .select()
      .from(target_exams)
      .where(eq(target_exams.category_id, id));
  ...
  async updateCategory(id: string, data: any) {
    ...
    if (data.name || data.slug) {
      const slug =
        data.slug || (data.name ? ConvertInSlug(data.name) : undefined);
