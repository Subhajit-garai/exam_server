...
export const subscriptionInputZodSchema = z.object({
  ...
  plan: z.enum(primeStatus.enumValues).optional(), // Use plan name like BASIC, STANDARD
  offerActive: z.array(z.string()).default([]),
  offerInActive: z.array(z.string()).default([]),
  btncolor: z.string().optional(),
  type: z.literal("SUBSCRIPTION").default("SUBSCRIPTION"),
});

export const subscriptionUpdateZodSchema = subscriptionInputZodSchema
  .partial()
  .extend({
