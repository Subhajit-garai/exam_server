...
export const bulkMockAndPyqCreate = async (bulkData: any[]) => {
  ...
  for (const data of bulkData) {
    try {
      // Move logs outside transaction
      // console.log("Processing data.mockSetId:", data.mockSetId);
      // console.log("Processing test --->:", data.name);

      const mockset = await prisma.mock_questions_set.findFirst({
          where: { id: data.mockSetId },
        });
        if (!mockset) throw new Error("Mock set not found");

        const exampattern = await prisma.exam_pattern.findFirst({
          where: { title: mockset.pattern },
        });
...
export const bulk_mock_questions_set_Create = async (bulkData: any) => {
  const result = await prisma.mock_questions_set.createMany({
    data: bulkData,
