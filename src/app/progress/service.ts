...
import { topics, subjects } from "@repo/db/schema/note.js";
import { user_topic_progress, exam_progress, dpp_progress, quiz_progress } from "@repo/db/schema/progress.js";
import { syllabuses, subject_syllabus_maps } from "@repo/db/schema/syllabus.js";
import { exams } from "@repo/db/schema/exam.js";
import { scores } from "@repo/db/schema/score.js";
import { eq, and, inArray, sum, count as drizzleCount, desc as drizzleDesc } from "drizzle-orm";
...
export class ProgressService {
    async trackTopicProgress(userId: string, topicName: string, timeSpentDelta: number) {
        ...
        const [existing] = await db.select().from(user_topic_progress).where(and(
        ...
        if (existing) {
            const [updated] = await db.update(user_topic_progress).set({
                time_spent: newTimeSpent,
                last_read_at: new Date(),
                status: status !== "COMPLETED" ? "IN_PROGRESS" : "COMPLETED"
            }).where(eq(user_topic_progress.id, existing.id)).returning();
            return updated;
        } else {
            const [created] = await db.insert(user_topic_progress).values({
                user_id: userId,
                topic_id: topic.id,
    ...
    async getSyllabusProgress(userId: string, examYearId: string) {
        const [syllabusData] = await db.select().from(syllabuses).where(and(
        ...
        const subjectMaps = await db.select().from(subject_syllabus_maps).where(eq(subject_syllabus_maps.syllabus_id, syllabusData.id));

        const subjectMapsWithDetails = await Promise.all(subjectMaps.map(async (sm) => {
            const [subject] = await db.select().from(subjects).where(eq(subjects.id, sm.subject_id)).limit(1);
            if (!subject) return null;
            const topicList = await db.select({ id: topics.id, name: topics.name, subjectId: topics.subject_id }).from(topics).where(eq(topics.subject_id, subject.id));
            return { ...sm, subject: { ...subject, topics: topicList } };
        }));

        const syllabus = { ...syllabusData, subjectMaps: subjectMapsWithDetails.filter(Boolean) };

        if (!syllabus) return { totalProgress: 0, subjects: [] };

        const allSubjects = (syllabus.subjectMaps as any).map((map: any) => map.subject);
        ...
        for (const subject of allSubjects) {
            ...
            const completedCountResult = await db.select({ value: drizzleCount() }).from(user_topic_progress).where(and(
                eq(user_topic_progress.user_id, userId),
    ...
    async getStudyHours(userId: string) {
        ...
        const progressAggregate = await db.select({ value: sum(user_topic_progress.time_spent) }).from(user_topic_progress).where(eq(user_topic_progress.user_id, userId));
    ...
    private async _getExamProgress(userId: string) {
        const [progress] = await db.select().from(exam_progress).where(eq(exam_progress.user_id, userId)).limit(1);
        return progress;
    }
    ...
    async updateUserProgressAfterExam(userId: string, examId: string, evaluationResult?: any) {
        await this.updateExamProgress(userId, examId);
    }

    async updateExamProgress(userId: string, examId: string) {
        const [exam] = await db.select({ examtype: exams.exam_type }).from(exams).where(eq(exams.id, examId)).limit(1);

        if (!exam) return;

        const [scoreEntry] = await db.select({ result: scores.result }).from(scores).where(and(eq(scores.exam_id, examId), eq(scores.user_id, userId))).limit(1);
        ...
        switch (exam.examtype) {
            case "Dpp": {
                const [existing] = await db.select().from(dpp_progress).where(eq(dpp_progress.user_id, userId)).limit(1);
                if (existing) {
                    await db.update(dpp_progress).set({
                ...
                break;
            case "Quiz": {
                const [existing] = await db.select().from(quiz_progress).where(eq(quiz_progress.user_id, userId)).limit(1);
                if (existing) {
                    await db.update(quiz_progress).set({
            ...
            default: {
                const [existing] = await db.select().from(exam_progress).where(eq(exam_progress.user_id, userId)).limit(1);
                let updatedProgress: any;
                if (existing) {
                    const [updated] = await db.update(exam_progress).set({
                        attended: (existing.attended || 0) + 1,
                        total_correct: (existing.total_correct || 0) + right,
                        total_questions_attempted: (existing.total_questions_attempted || 0) + totalAttemptedInThisExam,
                        last_exam_id: examId,
                        last_exam_date: new Date()
                    }).where(eq(exam_progress.user_id, userId)).returning();
                    updatedProgress = updated;
                } else {
                    const [created] = await db.insert(exam_progress).values({
                        user_id: userId,
                        attended: 1,
                        total_correct: right,
                ...
                if (updatedProgress.total_questions_attempted > 0) {
                    ...
                    await db.update(exam_progress).set({ accuracy: newAccuracy }).where(eq(exam_progress.user_id, userId));
                }
            }
                break;
        }
