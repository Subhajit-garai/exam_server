...
import { UserRole, SocialPlatform, primeStatus } from "@/db/enums.js";
...
export const users = pgTable("users", {
  ...
  reset_token_expires: timestamp("reset_token_expires", { precision: 3 })
    .notNull()
    .defaultNow(),
...
export const socials = pgTable(
  ...
  (table) => ({
    user_platform_unique_idx: uniqueIndex("socials_user_id_platform_key").on(
      table.user_id,
      table.platform,
    ),
    platform_link_unique_idx: uniqueIndex("socials_platform_link_key").on(
      table.platform,
      table.link,
    ),
  }),
...
export const user_answers = pgTable(
  ...
  (table) => ({
    exam_user_question_unique_idx: uniqueIndex(
      "user_answers_exam_id_user_id_question_id_key",
    ).on(table.exam_id, table.user_id, table.question_id),
  }),
);
