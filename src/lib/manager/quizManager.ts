...
export class QuizManager {
    ...
    async CreateQuiz(userid: string, userRole: string, data: activity_quiz_create_data_type) {
        ...
        switch (mode) {
            case "1v1": limit = 2; break;
            case "1v2": limit = 3; break;
            case "1v3": limit = 4; break;
            case "1v4": limit = 5; break;
            case "1v5": limit = 6; break;
            default: throw new CustomError("Free mode not implemented yet", 501);
        }
    ...
    async addUser(quizId: string, userId: string, name: string, avatar?: string | null) {
    ...
    async startQuiz(quizId: string) {
        ...
        await this.redis.set(`quiz:startTime:${quizId}`, startTime.toISOString(), "EX", 7200);
    ...
    private scheduleQuizTimer(quizId: string, delay: number, questionIndex: number) {
    ...
    async sendQuestionToRoom(quizId: string, questionNumber: number, startTime: Date, endTime: Date) {
        ...
        await this.redis.set(`quiz:question:startTime:${quizId}:${questionNumber}`, startTime.toISOString(), "EX", 300);
        await this.redis.set(`quiz:question:endTime:${quizId}:${questionNumber}`, endTime.toISOString(), "EX", 300);
    ...
    async submitAnswer(quizId: string, userId: string, userans: string[], number: number, isMultiple: boolean, submissionTimeIso: string) {
        const alreadyAnswered = await this.redis.hexists(`quiz:submissions:${quizId}:${userId}`, number.toString());
