...
import { syllabuses, subject_syllabus_maps, topic_subject_maps } from "@repo/db/schema/syllabus.js";
import { subjects, topics } from "@repo/db/schema/note.js";
import { eq, and } from "drizzle-orm";
import { exam_years, target_exams } from "@repo/db/schema/exam.js";
import {
...
export class SyllabusService {
    async createSyllabus(data: any) {
        ...
        const [response] = await db.insert(syllabuses).values({
            ...data,
        }).returning();
    ...
    async getSyllabusById(syllabusid: string) {
        const [response] = await db.select().from(syllabuses).where(eq(syllabuses.id, syllabusid));
    ...
    async deleteSyllabus(id: string) {
        const [response] = await db.delete(syllabuses).where(eq(syllabuses.id, id)).returning();
        if (!response) throw Error("error while deleting syllabus");
        return response;
    }

    async getSyllabusByExamYearId(id: string) {
        const response = await db.select().from(syllabuses).where(eq(syllabuses.exam_year_id, id));
        if (!response) throw Error("error while fetching all syllabus");
        return response;
    }
    ...
    async getSyllabusName() {
        const rows = await db.select({
            ...
            .from(syllabuses)
            .leftJoin(subject_syllabus_maps, eq(syllabuses.id, subject_syllabus_maps.syllabus_id))
        ...
        rows.forEach(row => {
            ...
            if (row.subject && row.subject.name) {
                syllabusMap.get(row.id).SubjectSyllabusMap.push({ subject: row.subject });
            }
        });
    ...
    async addSubject(data: any, by: "id" | "name" | "shortname" = "id") {
        ...
        switch (by) {
            case "name": {
                ...
                const [inserted] = await db.insert(subject_syllabus_maps).values({ ...rest, subject_id: subject.id }).returning();
            ...
            case "shortname": {
                ...
                const [subject] = await db.select().from(subjects).where(eq(subjects.short_name, shortName));
            ...
            default: {
                ...
                const [inserted] = await db.insert(subject_syllabus_maps).values({ ...processedata.data }).returning();
                Subject_added = inserted;
                break;
            }
        }
    ...
    async removeSubject(syllabusid: string, subjectid: string) {
        const [Subject_removed] = await db.delete(subject_syllabus_maps).where(
            and(
    ...
    async addTopic(data: any, by: "id" | "name" | "shortname" = "id") {
        ...
        switch (by) {
            case "name": {
                ...
                const [subject_map] = await db.select().from(subject_syllabus_maps).where(
                    and(eq(subject_syllabus_maps.syllabus_id, syllabus_id), eq(subject_syllabus_maps.subject_id, subject_id))
                );
                ...
                const [inserted] = await db.insert(topic_subject_maps).values({ topic_id: topic.id, weightage: weightage, subject_map_id: subject_map.id }).returning();
            ...
            case "shortname": {
                ...
                let { shortName, subject_id, syllabus_id, weightage } = processedata.data;
                const [subject_map] = await db.select().from(subject_syllabus_maps).where(
                    and(eq(subject_syllabus_maps.syllabus_id, syllabus_id), eq(subject_syllabus_maps.subject_id, subject_id))
                );
                if (!subject_map) throw Error("syllabus doesnot have selected sullabus");

                const [topic] = await db.select().from(topics).where(eq(topics.short_name, shortName));
            ...
            default: {
                ...
                let { topic_id, subject_id, syllabus_id, weightage } = processedata.data;
                const [subject_map] = await db.select().from(subject_syllabus_maps).where(
                    and(eq(subject_syllabus_maps.syllabus_id, syllabus_id), eq(subject_syllabus_maps.subject_id, subject_id))
                );
                if (!subject_map) throw Error("syllabus doesnot have selected sullabus");

                const [inserted] = await db.insert(topic_subject_maps).values({ topic_id: topic_id, weightage: weightage, subject_map_id: subject_map.id }).returning();
                Topic_added = inserted;
                break;
            }
        }
    ...
    async removeTopic(syllabusId: string, subjectId: string, topicId: string) {
        ...
        const [Topic_removed] = await db.delete(topic_subject_maps).where(
            and(
        ...
        if (!Topic_removed) throw Error("error while topic deletion for sullabus subject");
    ...
    async getFormattedSyllabus(exam_year_id?: string, syllabusid?: string) {
        ...
        const fetchSyllabus = async (whereClause: any) => {
            const [baseSyllabus] = await db.select({
                id: syllabuses.id,
            ...
            const subjectMaps = await db.select({
                ...
                .leftJoin(subjects, eq(subject_syllabus_maps.subject_id, subjects.id))
                .where(eq(subject_syllabus_maps.syllabus_id, baseSyllabus.id as string));

            const subjectMapsWithTopics = await Promise.all(subjectMaps.map(async (sm) => {
    ...
    async getDetaildformatedSyllabus(syllabusid?: string) {
        ...
        const [baseSyllabus] = await db.select().from(syllabuses).where(eq(syllabuses.id, syllabusid)).limit(1);
