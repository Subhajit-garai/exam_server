...
import { topics, subjects, categories, topic_note_versions } from "@repo/db/schema/note.js";
import { user_topic_progress } from "@repo/db/schema/progress.js";
...
export class NoteService {
    async like(topicId: string) {
        const [topicData] = await db.select().from(topics).where(eq(topics.slug, topicId));
        ...
        return await db.transaction(async (tx) => {
            ...
            const [updatedContent] = await tx.update(topics)
    ...
    async dislike(topicSlug: string) {
        const [topicData] = await db.select().from(topics).where(eq(topics.slug, topicSlug));
        if (!topicData) throw new Error("Topic data not found in database");
    ...
    async updateContentOfTopic(topicId: string, newContent: any) {
        return await db.transaction(async (tx) => {
            const [topicData] = await tx.select().from(topics).where(eq(topics.id, topicId));
            if (!topicData) throw new Error("Topic data not found in database");
    ...
    async deleteTopic(id: string) {
        const [isExist] = await db.select({ id: topics.id }).from(topics).where(eq(topics.id, id));
    ...
    async deleteSubject(id: string) {
        const [isExist] = await db.select({ id: subjects.id }).from(subjects).where(eq(subjects.id, id));
    ...
    async getAllVersionOfNote(topicSlug: string) {
        const [topicdata] = await db.select({ id: topics.id }).from(topics).where(eq(topics.slug, topicSlug));
        if (!topicdata) throw new Error("Topic not found");

        const versions = await db.select().from(topic_note_versions).where(eq(topic_note_versions.topic_id, topicdata.id));
        return versions;
    }

    async getAllNoteTopic(slug: string) {
        const [subjectdata] = await db.select({ id: subjects.id }).from(subjects).where(eq(subjects.slug, slug));
        ...
        }).from(topics).where(eq(topics.subject_id, subjectdata.id));
    ...
    async getAllNoteSubjectForUser(exam_id?: string, exam_year_id?: string) {
        if (exam_id && exam_year_id) {
            const syllabiData = await db.select({ id: syllabuses.id }).from(syllabuses).where(eq(syllabuses.exam_year_id, exam_year_id));
            const syllabusIds = syllabiData.map((s) => s.id);

            if (syllabusIds.length === 0) return [];

            const subjectMaps = await db.select({ subject: subjects })
    ...
    async getAllNoteSubjectByCategory(category?: string) {
        if (category) {
            const subjectList = await db.select({ subject: subjects })
                .from(subjects)
    ...
    async getAllNoteSubjectByExam(exam?: string) {
        if (exam) {
            const [targetExam] = await db.select({ id: target_exams.id }).from(target_exams).where(
                or(
            ...
            const examYearsData = await db.select({ id: exam_years.id }).from(exam_years).where(eq(exam_years.target_exam_id, targetExam.id));
            const examYearIds = examYearsData.map((ey) => ey.id);

            if (examYearIds.length === 0) return [];

            const syllabiData = await db.select({ id: syllabuses.id }).from(syllabuses).where(inArray(syllabuses.exam_year_id, examYearIds));
            const syllabusIds = syllabiData.map((s) => s.id);
    ...
    async getTopic(topicId: string) {
        const [note] = await db.select({
            ...
            attachments: topics.attachments,
            status: topics.status,
        }).from(topics).where(eq(topics.id, topicId));
        return note;
    }
    ...
    async getNote(subjectSlug: string, topicSlug: string, userId: string) {
        ...
        const [topicProgress] = await db.select().from(user_topic_progress).where(
            and(
