...
export const createDpp = async (event: events) => {
  ...
  if (event.type == eventType.CREATE_DPP) {
    ...
    if (event.data.name === "autoincrement") {
      ...
      for (let index = 0; index < create_exam_count_for_date.length; index++) {
        ...
        for (let idx = 0; idx < create_exam_count_for_date[index]; idx++) {
          temp_name_array.push(`Dpp@${new_exam_number}`);
          new_exam_number++;
        }
    ...
    // console.log("-------->",event.data.exam_pattern);

    if (event.data.exam_pattern) {
      let is_exam_pattern_id_valid = await prisma.exam_pattern.findMany({
        where: {
          id: {
            in: event.data.exam_pattern,
          },
        },
        select: {
          id: true,
      ...
      if (!is_exam_pattern_id_valid) {
        ...
        let get_exam_pattern_id = await prisma.exam_pattern.findMany({
          where: {
            title: {
              startsWith: "JECA@PATTERN@",
            },
          },
          select: {
            id: true,
        ...
        if (get_exam_pattern_id) {
          event.data.exam_pattern = get_exam_pattern_id.map((item) => item.id);
        } else {
          throw new Error(
    ...
    for (let index = 0; index < create_exam_count_for_date.length; index++) {
      for (let idx = 0; idx < create_exam_count_for_date[index]; idx++) {
        let exam_pattern_id: string = "";
        let num = Math.floor(Math.random() * event.data.exam_pattern.length);
        exam_pattern_id = event.data.exam_pattern[num];
        event.data.exam_pattern.splice(num, 1);

        console.log("selected exam_pattern_id is ", exam_pattern_id);
