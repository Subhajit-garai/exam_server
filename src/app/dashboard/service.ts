...
import { eq, gte, and, sum, count as drizzleCount, desc as drizzleDesc } from "drizzle-orm";
...
export class DashboardService {
    ...
    async getPaymentStats(range: string) {
        ...
        const paymentsData = await db.select().from(payments).where(and(
            gte(payments.created_at, startDate),
            eq(payments.status, "captured")
        ));

        const totalRevenue = paymentsData.reduce((sum: number, p: any) => sum + (p.amount / 100), 0);
    ...
    async getActiveUserCount() {
        ...
        const onlineCountResult = await db.select({ value: drizzleCount() }).from(users).where(eq(users.is_online, true));
        const onlineCount = onlineCountResult[0].value;

        const recentlyActiveCountResult = await db.select({ value: drizzleCount() }).from(users).where(gte(users.last_seen, fifteenMinutesAgo));
    ...
    async getKeyMetrics() {
        const totalUsersResult = await db.select({ value: drizzleCount() }).from(users).where(eq(users.role, "User"));
        const totalUsers = totalUsersResult[0].value;

        const totalRevenueResult = await db.select({ value: sum(payments.amount) }).from(payments).where(eq(payments.status, "captured"));
        const totalRevenue = parseInt(totalRevenueResult[0].value || "0") / 100;

        const totalOrdersResult = await db.select({ value: drizzleCount() }).from(orders);
    ...
    async getPaymentHistory(page: number = 1, limit: number = 10, range: string = "all") {
        ...
        const totalDocsResult = await db.select({ value: drizzleCount() }).from(payments).where(gte(payments.created_at, startDate));
