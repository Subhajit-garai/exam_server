...
export const question_maps = pgTable(
  "question_maps",
  {
    id: text("id")
      .notNull()
  ...
  (table) => ({
    exam_question_part_unique_idx: uniqueIndex(
      "question_maps_exam_id_question_id_part_key",
    ).on(table.exam_id, table.question_id, table.part),
  }),
);
...
export const question_processing = pgTable("question_processing", {
  ...
  created_by: text("created_by")
    ...
    .references(() => users.id, { onDelete: "cascade", onUpdate: "cascade" }),
  created_at: timestamp("created_at", { precision: 3 }).notNull().defaultNow(),
  question_id: text("question_id").references(() => questions.id, {
    onDelete: "set null",
    onUpdate: "cascade",
  }),
  ...
  processed_by: text("processed_by").references(() => users.id, {
    onDelete: "set null",
    onUpdate: "cascade",
  }),
...
export const question_processingRelations = relations(
  question_processing,
  ({ one }) => ({
    topic: one(topics, {
