...
export class BotExamService {
    ...
    async checkExamCompletionStatus(examid: string) {
        ...
        await Promise.all(
            examData.exam_pattern.total_questions.map(async (expectedCount: number, idx: number) => {
    ...
    async getQuestionsIds() {
        const topicNormalAnsQuestions: QuestionsId[] = await prisma.$queryRawUnsafe(`SELECT  s.name AS subject_name, ARRAY_AGG(q.id) AS ids FROM "Question" q JOIN "Subject" s ON q.subject_id = s.id  WHERE is_multiple_ans = false AND status = 'Done' GROUP BY s.name;`);
        const topicMultiplaAnsQuestions: QuestionsId[] = await prisma.$queryRawUnsafe(`SELECT  s.name AS subject_name, ARRAY_AGG(q.id) AS ids FROM "Question" q JOIN "Subject" s ON q.subject_id = s.id  WHERE is_multiple_ans = true AND status = 'Done' GROUP BY s.name;`);
