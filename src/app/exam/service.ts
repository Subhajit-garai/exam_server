import { ExamType, Visibility } from "@repo/db/schema/enums.js";
import { db } from "@repo/db/index.js";
import { exams, exam_patterns, exam_years, target_exams } from "@repo/db/schema/exam.js";
import { socials, user_answers } from "@repo/db/schema/user.js";
import { scores, leaderboards } from "@repo/db/schema/score.js";
import { categories, subjects, topics } from "@repo/db/schema/note.js";
import { contest_registers } from "@repo/db/schema/schema.js";
import { questions } from "@repo/db/schema/question.js";
import { eq, and, desc as drizzleDesc, or, gte, lte, count as drizzleCount, asc as drizzleAsc } from "drizzle-orm";
...
export class ExamService {
    ...
    async getUserMetaDataForExam(userId: string, examId: string) {
        ...
        const [userScore] = await db.select({ score: scores.score, result: scores.result }).from(scores).where(and(eq(scores.user_id, userId), eq(scores.exam_id, examId))).limit(1);

        const [userLeaderboard] = await db.select({ rank: leaderboards.rank }).from(leaderboards).where(and(eq(leaderboards.user_id, userId), eq(leaderboards.exam_id, examId))).limit(1);

        const [topper] = await db.select({ user_id: leaderboards.user_id, score: leaderboards.score }).from(leaderboards).where(and(eq(leaderboards.exam_id, examId), eq(leaderboards.rank, 1))).limit(1);
    ...
    async getCategoryName() {
        const response = await db.select({ name: categories.name }).from(categories);
    ...
    async getExamAttemptQuestionMetaData(userId: string, examId: string) {
        const [data] = await db.select({ not_attempt: scores.not_attempt, total_questions: scores.total_questions }).from(scores).where(and(eq(scores.user_id, userId), eq(scores.exam_id, examId))).limit(1);
        return data;
    }
        ...
        if (exam.creation_status === "Done") {
            if (exam.examtype !== "Mock" && exam.examtype !== "PYQ") {
                if (isUserGivenThisExam?.id) {
                    throw new CustomError("You have already taken this exam. Please join the next one.");
                }
    ...
    async createExam(data: any, userId: string) {
        ...
        return await db.transaction(async (tx) => {
            const [register] = await tx.insert(contest_registers).values({ count: 0 }).returning();
    ...
    async getExamsById(id: string) {
        ...
        return rawExam.map(row => ({
            ...
            ContestRegister: row.contestRegisterCount !== null ? {
    ...
    async getExams(
        ...
        const totalResult = await db.select({ value: drizzleCount() }).from(exams).where(and(...filters));
    ...
    private isExamSessionActive(exam: any) {
        ...
        if (isSame) {
            ...
            if (started) {
                ...
                let isExamJoinTimeExecd = joinTimeLimit ? currentISTTime.isAfter(joinTimeLimit) : false;
    ...
    async updateExam(id: string, updateData: any) {
        const [response] = await db.update(exams).set({
            ...
            ...(updateData.creationstatus && { creation_status: updateData.creationstatus }),
    ...
    async getOnlyExamById(id: string) {
        const rawResponse = await db.select()
            .from(exams)
            .leftJoin(exam_patterns, eq(exams.exam_pattern_id, exam_patterns.id))
            .leftJoin(contest_registers, eq(exams.register_id, contest_registers.id))
            .where(eq(exams.id, id));
    ...
    async deleteExamResult(userId: string, examId: string) {
        const isAvailable = await db.select({ id: user_answers.id }).from(user_answers)
            .where(and(eq(user_answers.user_id, userId), eq(user_answers.exam_id, examId)))
            .limit(1);

        if (isAvailable.length > 0) {
            await db.delete(user_answers).where(and(eq(user_answers.user_id, userId), eq(user_answers.exam_id, examId)));
            await db.delete(scores).where(and(eq(scores.user_id, userId), eq(scores.exam_id, examId)));
            await db.delete(leaderboards).where(and(eq(leaderboards.user_id, userId), eq(leaderboards.exam_id, examId)));
            return "reset done";
        }
