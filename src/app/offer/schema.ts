...
export const subscription_offers = pgTable("subscription_offers", {
  ...
  target_exam_id: text("target_exam_id").references(() => target_exams.id, {
    onDelete: "cascade",
    onUpdate: "cascade",
  }),
  tier_id: text("tier_id").references(() => tiers.id, {
    onDelete: "cascade",
    onUpdate: "cascade",
  }),
  time: text("time"),
...
export const subscription_offersRelations = relations(
  subscription_offers,
  ({ one }) => ({
