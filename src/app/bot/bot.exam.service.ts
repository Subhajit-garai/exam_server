...
export class BotExamService {
  ...
  async checkExamCompletionStatus(examid: string) {
    const examDataResult = await db
      ...
      .from(schema.exams)
      .innerJoin(
        schema.exam_patterns,
        eq(schema.exams.exam_pattern_id, schema.exam_patterns.id),
      )
    ...
    if (examData.creationstatus === "Done")
      throw new Error("Exam status already checked");

    // Verify question counts for each part
    await Promise.all(
      (examData.total_questions as number[]).map(
        async (expectedCount: number, idx: number) => {
  ...
  async getSyllabusDataForExamCreation(syllabusid: string) {
    ...
    const results = await db
      ...
      .innerJoin(
        schema.subjects,
        eq(schema.subject_syllabus_maps.subject_id, schema.subjects.id),
      )
  ...
  async getQuestionDetailsforProcessing(data: any) {
    ...
    let isAlreadyInProcessing = await db
      .select()
    ...
    if (isAlreadyInProcessing && isAlreadyInProcessing.length !== 0) {
      ...
      const processingIds = new Set(
        isAlreadyInProcessing.map((item) => item.question_id),
      );
      finalSelectedQuestion = questionData.filter(
        (question) => !processingIds.has(question.id),
      );
    } else {
    ...
    if (!finalSelectedQuestion || finalSelectedQuestion.length === 0)
  ...
  async addProcessingQuestions(data: any[]) {
    ...
    const result = await db
      .insert(schema.question_processing)
      .values(
        data.map((item) => ({
          title: item.title,
  ...
  async getQuestionDetailsForBot(ids: string[]) {
    const questionData = await db
      ...
      .leftJoin(
        schema.subjects,
        eq(schema.questions.subject_id, schema.subjects.id),
      )
      .where(inArray(schema.questions.id, ids));

    if (!questionData || questionData.length === 0)
      throw new Error("Questions not found or invalid IDs");
  ...
  async getQuestionsIds() {
    const topicNormalAnsQuestionsResult = await db.execute(
      sql`SELECT  s.name AS subject_name, ARRAY_AGG(q.id) AS ids FROM "questions" q JOIN "subjects" s ON q.subject_id = s.id  WHERE is_multiple_answers = false AND status = 'Done' GROUP BY s.name;`,
    );
    const topicMultiplaAnsQuestionsResult = await db.execute(
      sql`SELECT  s.name AS subject_name, ARRAY_AGG(q.id) AS ids FROM "questions" q JOIN "subjects" s ON q.subject_id = s.id  WHERE is_multiple_answers = true AND status = 'Done' GROUP BY s.name;`,
    );

    const topicNormalAnsQuestions: QuestionsId[] =
      topicNormalAnsQuestionsResult.rows as unknown as QuestionsId[];
    const topicMultiplaAnsQuestions: QuestionsId[] =
      topicMultiplaAnsQuestionsResult.rows as unknown as QuestionsId[];
  ...
  async getQuestionsByIds(ids: string[]) {
    ...
    if (!questions || questions.length === 0)
      throw new Error("Questions not found for given ids");
    return questions;
  }

  async getQuestionsInfoForExam(examid: string) {
    const questionMapData = await db
      .select()
      .from(schema.question_maps)
      .where(eq(schema.question_maps.exam_id, examid));

    if (!questionMapData || questionMapData.length === 0)
  ...
  async addQuestionsToExam(questions: any[]) {
    return await db.insert(schema.question_maps).values(
      questions.map((q) => ({
        number: q.number,
  ...
  async getExamPatternId(examid: string) {
    const data = await db
      .select({ exam_pattern_id: schema.exams.exam_pattern_id })
      .from(schema.exams)
      .where(eq(schema.exams.id, examid))
      .limit(1);
  ...
  async getExamAnswers(examid: string) {
    const questions_map_data = await db
      .select()
      .from(schema.question_maps)
      .where(eq(schema.question_maps.exam_id, examid));

    if (!questions_map_data || questions_map_data.length === 0)
      throw new Error("Exam invalid or doesn't have any ans");
