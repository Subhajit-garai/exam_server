...
export class ProfileService {
    async getProfile(userId: string) {
        ...
        const [primeData] = await db.select({
    ...
    async updateAcademicProfile(userId: string, data: UpdateProfileInput) {
        ...
        if (exam) {
            const [targetExam] = await db.select().from(target_exams).where(eq(target_exams.short_code, exam)).limit(1);

            if (targetExam) {
                updateData.targeted_exam_id = targetExam.id;

                if (year) {
                    const [targetExamYear] = await db.select().from(exam_years).where(and(
                    ...
                    if (targetExamYear) {
                        ...
                        const [latestExamYear] = await db.select().from(exam_years).where(eq(exam_years.target_exam_id, targetExam.id)).orderBy(drizzleDesc(exam_years.year)).limit(1);
    ...
    async updateSocialLinks(userId: string, data: UpdateSocialLinksInput) {
        if (data.link) {
            const [existingSocial] = await db.select().from(socials).where(and(
