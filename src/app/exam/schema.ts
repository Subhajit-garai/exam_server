...
import { contest_registers } from "@/db/schema.js";
...
export const target_examsRelations = relations(
  target_exams,
  ({ one, many }) => ({
...
export const exam_years = pgTable("exam_years", {
  id: text("id")
    ...
    .primaryKey()
    .$defaultFn(() => cuid()),
  target_exam_id: text("target_exam_id")
    .notNull()
  ...
  registration_close_date: timestamp("registration_close_date", {
    precision: 3,
  }),
...
export const exam_patterns = pgTable("exam_patterns", {
  ...
  is_multiple_answers: integer("is_multiple_answers")
    .array()
    .notNull()
    .default([0, 0]),
...
export const exam_patternsRelations = relations(
  exam_patterns,
  ({ one, many }) => ({
    category: one(categories, {
...
export const exams = pgTable("exams", {
  ...
  exam_pattern_id: text("exam_pattern_id")
    .notNull()
  ...
  creation_status: CreationTypes("creationstatus")
    .notNull()
    .default("Processing"),
...
export const exam_timelines = pgTable("exam_timelines", {
  ...
  created_at: timestamp("created_at", { precision: 3 }).notNull().defaultNow(),
  updated_at: timestamp("updated_at", { precision: 3 }).notNull(),
  exam_year_id: text("exam_year_id")
    .notNull()
