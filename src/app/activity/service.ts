...
export class ActivityService {
  ...
  async getDailyChallenge() {
    ...
    let [challenge] = await db
      ...
      .where(eq(daily_challenges.date, today))
      .limit(1);
  ...
  async getDailyChallengeHistory() {
    return db
      .select()
      .from(daily_challenges)
      .orderBy(drizzleDesc(daily_challenges.date))
      .limit(20);
  }
  ...
  async getXPLeaderboard(
    type: activity_time_range = "weekly",
    limit: number = 10,
  ) {
  ...
  async getUserXP(userId: string): Promise<number> {
    const globalScore = await this.redis.zscore(
      "leaderboard:xp:global",
      userId,
    );
  ...
  async updateStreak(userId: string) {
    ...
    const lastActivityDate = streakRecord.last_activity
      ? dayjs(streakRecord.last_activity).startOf("day")
      : null;
  ...
  private async assignBadge(
    ...
    const [existing] = await db
      ...
      .where(
        and(
          ...
          eq(user_badges.badge_id, badge.id),
        ),
      )
      .limit(1);
    ...
    if (!existing) {
      await db
        .insert(user_badges)
        .values({ user_id: userId, badge_id: badge.id });
    }
  }
  ...
  async createActivity(userId: string, data: CreateActivityInput, tx?: any) {
    ...
    const [newActivity] = await dbInstance
  ...
  async getRecentActivities(userId: string, limit: number = 10) {
    const activities = await db
      ...
      .from(user_activities)
      .where(eq(user_activities.user_id, userId))
      .orderBy(drizzleDesc(user_activities.date))
      .limit(limit);
